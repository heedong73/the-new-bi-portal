...
async def list_roles(db: SessionDep, _op=Depends(_require_users_menu)):
...
@router.post("/api/reports/{report_id}/permissions/bulk", response_model=list[PermissionResponse])
async def grant_permissions_bulk(report_id: int, body: PermissionBulkGrantRequest, db: SessionDep, op=Depends(_require_reports_menu)):
    """한 주체(user/role/dept/group)에 여러 권한을 한 번에 부여(멱등).

    이미 있는 권한은 건너뛰고 없는 것만 생성한다. 응답은 해당 레포트의 전체 권한 목록.
    """
    report = await db.scalar(select(Report).where(Report.id == report_id))
    if report is None:
        raise NotFoundError("레포트를 찾을 수 없습니다.")
    ...
    if added:
        ...
        await append_audit(db, action=AuditAction.PERMISSION_CHANGE, result="success",
                           actor_user_id=op["user_id"], actor_label=op["emp_no"],
                           resource_type="report", resource_id=str(report_id),
                           meta={"target": "grant_bulk", "subject_type": str(body.subject_type),
                                 "subject_id": body.subject_id, "permissions": added})
    await db.commit()
