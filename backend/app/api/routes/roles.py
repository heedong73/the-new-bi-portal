...
async def list_roles(db: SessionDep, _op=Depends(_require_operator)):
...
async def assign_role(user_id: int, body: RoleAssignRequest, db: SessionDep, op=Depends(_require_operator)):
    """역할 부여 (멱등)."""
...
async def revoke_role(user_id: int, role_code: str, db: SessionDep, op=Depends(_require_operator)):
...
async def list_permissions(report_id: int, db: SessionDep, _op=Depends(_require_operator)):
...
async def grant_permission(report_id: int, body: PermissionGrantRequest, db: SessionDep, op=Depends(_require_operator)):
...
async def revoke_permission(report_id: int, permission_id: int, db: SessionDep, op=Depends(_require_operator)):
    """레포트 권한 회수."""
    await db.execute(
