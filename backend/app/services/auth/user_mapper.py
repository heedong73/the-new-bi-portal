...
- 환경 설정 대상 사번: System_Operator 역할 멱등 부여
...
async def _ensure_system_operator_role(db: AsyncSession, user: User) -> None:
    """환경 설정에 등록된 사번이면 최고 관리자 역할을 중복 없이 부여한다."""
    configured_emp_nos = {
        emp_no.strip()
        for emp_no in settings.SYSTEM_OPERATOR_EMP_NOS.split(",")
        if emp_no.strip()
    }
    if user.external_id not in configured_emp_nos:
        return

    role = await db.scalar(
        select(Role).where(Role.code == RoleCode.SYSTEM_OPERATOR)
    )
    if role is None:
        return
...
async def map_user(db: AsyncSession, profile: HRProfile) -> User:
    ...
    설정된 사번에는 System_Operator 역할을 보장.
    """
    ...
    await _ensure_system_operator_role(db, user)
    return user
